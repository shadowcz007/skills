---
name: memory
description: 使用 mixlab Memory API 检索记忆（按 tag 表达式、生成关键词/tagExpr）与提交记忆（整理 source/category/content/tags 并 POST）。需 MEMORY_API_KEY、MEMORY_API_BASE_URL。触发：查记忆、检索记忆、记一下、提交记忆、memory、mixlab 记忆。
---

# Memory（记忆）

通过 mixlab Memory API 进行**检索记忆**（类似 grep，按 tag 查）和**提交记忆**（整理字段后 POST）。Base URL 与鉴权从环境变量读取。
...
根据用户意图生成可能的**关键词/tag**，再组合成 **tagExpr** 调用 API。
...
把用户口述或文本**整理成规范字段**再 POST，避免缺字段或格式混乱。
...
- **检索**：意图 → 候选 tag → 构建 tagExpr（AND/OR/括号）→ GET memories。
- **提交**：口述/文本 → 整理 source、category、content、tags → POST memories。
- 所有请求均使用环境变量 `MEMORY_API_KEY` 与 `MEMORY_API_BASE_URL`，不在技能中写死密钥。
