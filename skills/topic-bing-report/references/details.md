...
按需读取：域名白名单、查询示例、Playwright 正文抽取、报告模板、质量清单与排障。主流程见上级 `SKILL.md`。
...
| 话题方向 | 英文搜索词（示例） |
|----------|-------------------|
| 一人公司 / agent | `"AI agent" "solo founder" OR "one person company" (2024 OR 2025) -site:zhihu.com -site:csdn.net` |
| AI 模型进展 | `Claude Gemini comparison benchmark 2025 site:theverge.com OR site:arstechnica.com` |
| 品牌 AI 案例 | `brand AI case study 2024 site:techcrunch.com OR site:wired.com` |

### 1.5 多轮搜索与翻页

- 每个话题至少 **2～3 轮**不同 query：  
  1. 核心词 + `(2024 OR 2025 OR 2026)`  
  2. 细分 + `trend OR insight OR analysis`  
...
> 日期：YYYY-MM-DD
> 检索日：YYYY-MM-DD（执行 Bing 搜索当日，可与文章日期不同）
> 关键词：关键词1、关键词2、关键词3
...
- 观点注明 `[标题](URL)`；区分**原文发布日期**（若有）与**本次检索日期**。
- 英文材料中文转述，避免大段原文。
- 有判断与结构，非素材罗列。
- 字数 **1500～3000**。
